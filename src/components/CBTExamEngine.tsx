...
export const CBTExamEngine: React.FC<CBTExamEngineProps> = ({
  ...
  const handleFinalSubmit = useCallback(() => {
    ...
    // Overall scaled score out of 400 (standard OAU Post UTME format)
    const overallPercentage = totalQuestions > 0 ? Number(((totalCorrect / totalQuestions) * 100).toFixed(1)) : 0;
    const scaledScoreOutOf400 = totalQuestions > 0 ? Math.round((totalCorrect / totalQuestions) * 400) : 0;
    ...
    const result: TestResult = {
      ...
      totalAttempted,
      totalCorrect,
      totalScore: scaledScoreOutOf400,
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-2.5 flex flex-wrap items-center justify-between gap-3">
          ...
          <div className="flex items-center space-x-2 sm:space-x-3">
            ...
            <button
              ...
              className="flex items-center space-x-1 px-2.5 py-1.5 rounded-xl bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 text-xs font-semibold border border-slate-200 dark:border-slate-700"
            ...
            <button
              ...
              className="p-1.5 rounded-xl bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 text-xs font-bold"
            ...
            <button
              ...
              className="px-3.5 py-1.5 rounded-xl bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-700 hover:to-teal-700 text-white text-xs font-extrabold shadow-sm shadow-emerald-500/20 active:scale-95 transition-all"
            >
              Submit Exam
            </button>
            ...
            <div className="bg-white dark:bg-slate-900 rounded-2xl p-5 sm:p-7 border border-slate-200 dark:border-slate-800 shadow-sm min-h-[420px] flex flex-col justify-between">
                  ...
                  {currentQuestion?.options.map((optionText, optIdx) => {
                    ...
                    return (
                      <label
                        ...
                        className={`flex items-center space-x-3.5 p-3.5 sm:p-4 rounded-xl border-2 cursor-pointer transition-all duration-150 ${
                          isSelected
                            ? 'border-blue-600 bg-blue-50/80 dark:bg-blue-950/60 dark:border-blue-500 shadow-xs ring-1 ring-blue-500/20'
              ...
              <div className="flex items-center justify-between pt-6 mt-6 border-t border-slate-100 dark:border-slate-800">
                ...
                <button
                  ...
                  className={`flex items-center space-x-1.5 px-4 py-2 rounded-xl text-xs sm:text-sm font-bold border transition-colors ${
                    currentIndex === 0
                ...
                <button
                  ...
                  className={`flex items-center space-x-1.5 px-5 py-2 rounded-xl text-xs sm:text-sm font-bold transition-all ${
